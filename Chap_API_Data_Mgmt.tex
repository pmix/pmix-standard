...
Allocate memory for a \refapi{pmix_data_buffer_t} object and initialize it
...
\begin{arglist}
\argout{buffer}{Variable to be assigned the pointer to the allocated \refapi{pmix_data_buffer_t} (handle)}
\end{arglist}
...
Free a \refapi{pmix_data_buffer_t} object and the data it contains
...
\begin{arglist}
\argin{buffer}{Pointer to the \refapi{pmix_data_buffer_t} to be released (handle)}
\end{arglist}
...
Initialize a statically declared \refapi{pmix_data_buffer_t} object
...
\begin{arglist}
\argin{buffer}{Pointer to the allocated \refapi{pmix_data_buffer_t} that is to be initialized (handle)}
\end{arglist}
...
Release the data contained in a \refapi{pmix_data_buffer_t} object
...
\begin{arglist}
\argin{buffer}{Pointer to the \refapi{pmix_data_buffer_t} whose data is to be released (handle)}
\end{arglist}

%%%%
\descr

Free's the data contained in a \refapi{pmix_data_buffer_t} object
...
Load a blob into a \refapi{pmix_data_buffer_t} object
...
\begin{arglist}
\argin{buffer}{Pointer to a pre-allocated \refapi{pmix_data_buffer_t} (handle)}
...
Load the given data into the provided \refapi{pmix_data_buffer_t} object, usually done in preparation for unpacking the provided data. Note that the data is \textit{not} copied into the buffer - thus, the blob must not be released until after operations on the buffer have completed.
...
Unload the data from a \refapi{pmix_data_buffer_t} object
...
\begin{arglist}
\argin{buffer}{Pointer to the \refapi{pmix_data_buffer_t} whose data is to be extracted (handle)}
...
\begin{arglist}
\argin{target}{Pointer to a \refapi{pmix_proc_t} containing the nspace/rank of the process that will be unpacking the final buffer. A NULL value may be used to indicate that the target is based on the same \ac{PMIx} version as the caller. (handle)}
\argin{buffer}{Pointer to a \refapi{pmix_data_buffer_t} where the packed data is to be stored (handle)}
...
\argin{type}{The type of the data to be packed (\refapi{pmix_data_type_t})}
\end{arglist}
...
Unpack values from a \refapi{pmix_data_buffer_t}
...
\begin{arglist}
\argin{source}{Pointer to a \refapi{pmix_proc_t} structure containing the nspace/rank of the process that packed the provided buffer. A NULL value may be used to indicate that the source is based on the same \ac{PMIx} version as the caller. (handle)}
...
\argin{type}{The type of the data to be unpacked --- must be one of the \ac{PMIx} defined data types (\refapi{pmix_data_type_t})}
\end{arglist}
...
\argin{type}{The type of the data to be copied --- must be one of the PMIx defined data types. (\refapi{pmix_data_type_t})}
\end{arglist}
...
\argin{type}{The type of the data value to be printed --- must be one of the PMIx defined data types. (\refapi{pmix_data_type_t})}
\end{arglist}
...
\begin{arglist}
\argin{dest}{Pointer to the destination \refapi{pmix_data_buffer_t} (handle)}
\argin{src}{Pointer to the source \refapi{pmix_data_buffer_t} (handle)}
\end{arglist}
