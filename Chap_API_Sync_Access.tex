...
A successful return indicates that the request is being processed by the host environment and the result will be returned in the provided \refarg{cbfunc}. Note that the library must not invoke the callback function prior to returning from the \ac{API}.
...
The callback function \refarg{cbfunc} is only called when \refconst{PMIX_SUCCESS} is returned.
