...
%%% \reflabel{chap:api_kv_mgmt:access}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Put/Get and Put/Fence/Get Key-Value Sharing}
\label{chap:api_kv_mgmt:access}

In the Put/Get model of sharing data, key-values can be associated with one of the following realms:
\begin{itemize}
\item The session corresponding to a particular namespace or rank.
\item The job containing a particular namespace or rank.
\item The application containing a particular rank.
\item The node a particular rank is executing on.
\item A namespace.
\item A rank.   
\end{itemize}
...
A PMIx call can retrieve rank specific data by specifying the namespace and rank of the desired client 
using the pmix_proc_t parameter of PMIx_Get. 
To retrieve key-values associated with entities other than a specific rank is done using a combination of 
identifying a process or namespace and attributes.   The following table summarizes how to retrieve 
data from the entities listed above:

\begin{longtable}{ | p{1.5cm} | p{1cm} | p{3cm} | p{3.7cm} | p{3cm} |}
       \caption{How to retrieve key-values from data realms} \\
       \hline 
       \textbf{realm} & \textbf{nspace} & \textbf{rank} & \textbf{info ATTRIBUTE} & \textbf{info value} \\ \hline
       \endhead
       Session     & yes & yes      & PMIX_SESSION_INFO & true (bool) \\ \hline
       Job         & yes & WILDCARD & PMIX_JOB_INFO     & true (bool) \\ \hline
       Application & yes & yes      & PMIX_APP_INFO     & true (bool) \\ \hline
       Application &     &          & PMIX_APP_INFO     & true (bool) \\ 
                   &     &          & PMIX_APPNUM       & app number (uint32_t) \\ \hline
       Node        & yes & yes      & PMIX_NODE_INFO    & true (bool) \\ \hline
       Node        &     &          & PMIX_NODE_INFO    & true (bool) \\
                   &     &          & PMIX_HOSTNAME     & hostname (string) \\ \hline
       Node        &     &          & PMIX_NODE_INFO    & true (bool) \\ 
                   &     &          & PMIX_NODEID       & host number (uint32_t) \\ \hline
      Namespace    & yes & WILDCARD &                   &  \\ \hline
      Rank         & yes & yes      &                   &  \\ \hline
\end{longtable}
