...
% ISSUE175: Consider a background section for this inital material or just summarize briefly key points from this material and add to the a), b) c) list below(not bulleted list, next list down)
...
% ISSUE175: If we get rid of this section, we need to introduce PMI-1 and PMI-2 somewhere.
...
\item the physical scale of the machines, and the corresponding number of total processes they support, is expected to reach levels approaching  1 million processes executing across 100 thousand nodes. Prior methods for initiating applications relied on exchanging communication endpoint information between the processes, either directly or in some form of hierarchical collective operation. Regardless of the specific mechanism employed, the exchange across such large applications would consume considerable time, with estimates running in excess of 5-10 minutes; and
    %  ISSUE175: Perhaps this could be more about “scalability is a goal of this API” vs “PMI-previous didn’t scale well and this does(which is not supported by the text anyway)”
...
% ISSUE175:  If we get rid of this, make sure these things (SMS, WLM, RM, etc) are defined early on somewhere.
\end{itemize}

% ISSUE175: Consider presenting the full goals of the PMI efforts rather than presenting (as is currently done) PMIx as an add-on to PMI-previous.

% ISSUE175: Suggestion:  “PMIX is designed to support clusters of large sizes….” and then delete all text prior to this in the chapter?
...
% ISSUE175: Need to determine if PMI-1, PMI-2 API’s should be part of the standard (documented somehow?).  Or it is sufficient to say that these can easily be implemented on top of PMIx API’s?
