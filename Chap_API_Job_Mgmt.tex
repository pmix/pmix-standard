...
\priattr
The \refapi{PMIx_Log_nb} function in the \ac{PRI} does not parse or utilize the attributes directly. However, the attributes are passed to the host RM for processing.
...
\adviceuserstart
The \refapi{PMIx_Log_nb} API should \emph{never} be used by applications for streaming data as it is not a ``performant'' transport and can perturb the application since it involves the local \ac{PMIx} server and host \ac{SMS} daemon.
\adviceuserend
