...
%
\declareconstitemvalue{PMIX_ERR_JOB_EXE_NOT_FOUND}{-190}
Specified executable not found
%
\declareconstitemvalue{PMIX_ERR_JOB_INSUFFICIENT_RESOURCES}{-234}
Insufficient resources to spawn job
%
\declareconstitemvalue{PMIX_ERR_JOB_SYS_OP_FAILED}{-235}
System library operation failed
%
\declareconstitemvalue{PMIX_ERR_JOB_WDIR_NOT_FOUND}{-233}
...
%
\declareAttribute{PMIX_ENVARS_HARVESTED}{"pmix.evar.hvstd"}{bool}{
Environmental parameters have been harvested by the spawn requestor - the server
does not need to harvest them.
}
%
\declareAttribute{PMIX_JOB_TIMEOUT}{"pmix.job.time"}{int}{
...
%
\declareAttribute{PMIX_SPAWN_TIMEOUT}{"pmix.sp.time"}{int}{
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{App structure support functions}
The following functions are provided to support the \refstruct{pmix_app_t} structure.

%%%%%%%%%%%
\littleheader{Initialize the app structure}
\declareapiProvisional{PMIx_App_construct}

Initialize the \refstruct{pmix_app_t} fields

\copySignature{PMIx_App_construct}{6.0}{
void \\
PMIx_App_construct(pmix_app_t *m);
}
...
\declareapiProvisional{PMIx_App_destruct}

Destruct the \refstruct{pmix_app_t} fields

\copySignature{PMIx_App_destruct}{6.0}{
void \\
PMIx_App_destruct(pmix_app_t *m);
}
...
Returns pointer to the array of \refstruct{pmix_app_t} structures (handle)

%%%%%%%%%%%
\littleheader{Free an app structure}
\declareapiProvisional{PMIx_App_release}

Release a \refstruct{pmix_app_t} structure

\copySignature{PMIx_App_release}{6.0}{
void \\
PMIx_App_release(pmix_app_t *m);
}
...
\declareapiProvisional{PMIx_App_free}

Release an array of \refstruct{pmix_app_t} structures

\copySignature{PMIx_App_free}{6.0}{
void \\
PMIx_App_free(pmix_app_t *m,
...
\declareapiProvisional{PMIx_App_info_create}
...
\copySignature{PMIx_App_info_create}{6.0}{
void \\
PMIx_App_info_create(pmix_app_t *m,
\hspace*{24\sigspace}size_t n);
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Topology support functions}

The following functions support the \refstruct{pmix_topology_t} structure.

\littleheader{Initialize the topology structure}
\declareapiProvisional{PMIx_Topology_construct}

Initialize the \refstruct{pmix_topology_t} fields to \code{NULL}

\copySignature{PMIx_Topology_construct}{6.0}{
void \\
PMIx_Topology_construct(pmix_topology_t *m);
}
...
\declareapiProvisional{PMIx_Topology_create}

Allocate and initialize a \refstruct{pmix_topology_t} array.

\copySignature{PMIx_Topology_create}{6.0}{
pmix_topology_t* \\
PMIx_Topology_create(size_t n);
}
...
Returns pointer to the array of \refstruct{pmix_topology_t} structures (handle)


\littleheader{Release a topology array}
\declareapiProvisional{PMIx_Topology_free}

Release a \refstruct{pmix_topology_t} array.

\copySignature{PMIx_Topology_free}{6.0}{
void \\
PMIx_Topology_free(pmix_topology_t *p,
\hspace*{24\sigspace}size_t n);
}

\begin{arglist}
\argin{p} Pointer to the array of \refstruct{pmix_topology_t}
\argin{n}{Number of structures to be allocated (size_t)}
\end{arglist}
...
The following functions are provided to support the \refstruct{pmix_device_distance_t} structure.

%%%%
\littleheader{Initialize the device distance structure}
\declareapiProvisional{PMIx_Device_distance_construct}

Initialize the \refstruct{pmix_device_distance_t} fields.

\copySignature{PMIx_Device_distance_construct}{6.0}{
void \\
PMIx_Device_distance_construct(pmix_device_distance_t *m)
}
...
\declareapiProvisional{PMIx_Device_distance_destruct}

Destruct the \refstruct{pmix_device_distance_t} fields.

\copySignature{PMIx_Device_distance_destruct}{6.0}{
void \\
PMIx_Device_distance_destruct(pmix_device_distance_t *m)
}
...
\declareapiProvisional{PMIx_Device_distance_create}

Allocate and initialize a \refstruct{pmix_device_distance_t} array.

\copySignature{PMIx_Device_distance_create}{6.0}{
pmix_device_distance_t* \\
PMIx_Device_distance_create(size_t n);
}
...
Returns pointer to the array of \refstruct{pmix_device_distance_t} (handle)

%%%%
\littleheader{Release an device distance array}
\declareapiProvisional{PMIx_Device_distance_free}

Release an array of \refstruct{pmix_device_distance_t} structures.

\copySignature{PMIx_Device_distance_create}{6.0}{
void \\
PMIx_Device_distance_free(pmix_device_distance_t *m,
\hspace*{24\sigspace}size_t n);
}
