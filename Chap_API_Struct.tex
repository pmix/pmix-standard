...
\begin{constantdesc}
%
\declareconstitem{PMIX_ERR_JOB_FAILED_TO_START}
At least one process in the job failed to start
%
\declareconstitem{PMIX_ERR_JOB_APP_NOT_EXECUTABLE}
A specified application executable is does not have execution permissions
%
\declareconstitem{PMIX_ERR_JOB_NO_EXE_SPECIFIED}
No executable was specified in a spawn request
%
\declareconstitem{PMIX_ERR_JOB_FAILED_TO_MAP}
The \ac{RM} was unable to map (i.e., assign locations) the processes in the job
%
\declareconstitem{PMIX_ERR_JOB_CANCELLED}
The submitted job was canceled prior to launch
%
\declareconstitem{PMIX_ERR_JOB_FAILED_TO_LAUNCH}
The job failed to launch
%
\declareconstitem{PMIX_ERR_JOB_ABORTED}
The job was aborted due to at least one process calling \refapi{PMIx_Abort}
%
\declareconstitem{PMIX_ERR_JOB_KILLED_BY_CMD}
The job was killed in response to a user command
%
\declareconstitem{PMIX_ERR_JOB_ABORTED_BY_SIG}
The job was killed in response to a signal (e.g., \code{SIGTERM} or \code{SIGKILL})
%
\declareconstitem{PMIX_ERR_JOB_TERM_WO_SYNC}
The job was killed due to at least one process terminating without first calling \refapi{PMIx_Finalize}
%
\declareconstitem{PMIX_ERR_JOB_SENSOR_BOUND_EXCEEDED}
The job was killed due to at least one process exceeding a monitor threshold
%
\declareconstitem{PMIX_ERR_JOB_NEVER_LAUNCHED}
The job was never launched
%
\declareconstitem{PMIX_ERR_JOB_NON_ZERO_TERM}
The job was killed due to at least one process exiting with a non-zero status
%
\declareconstitem{PMIX_ERR_JOB_ALLOC_FAILED}
The job was unable to obtain an allocation
%
\declareconstitem{PMIX_ERR_JOB_CANNOT_LAUNCH}
...
Changed to \refconst{PMIX_EVENT_JOB_END}
%
...
The next set of constants are used to indicate that a job has abnormally terminated and to convey some information as to the cause:
...
The job was terminated due to one or more processes exiting without first calling \refapi{PMIx_Finalize}
%
...
\versionMarker{4.0}
The \refstruct{pmix_job_state_t} structure is a \code{uint8_t} type for job state values. The following constants can be used to set a variable of the type \refstruct{pmix_job_state_t}. All values were originally defined in version 4 of the standard unless otherwise marked.
...
\begin{constantdesc}
%
\declareconstitem{PMIX_JOB_STATE_UNDEF}
Undefined job state
%
\declareconstitem{PMIX_JOB_STATE_PREPPED}
Job is ready to be launched
%
\declareconstitem{PMIX_JOB_STATE_LAUNCH_UNDERWAY}
Job launch procedure is in progress
%
\declareconstitem{PMIX_JOB_STATE_RUNNING}
Job is running
%
\declareconstitem{PMIX_JOB_STATE_SUSPENDED}
Job has been suspended
%
\declareconstitem{PMIX_JOB_STATE_CONNECTED}
All processes in the job have connected to their \ac{PMIx} server
%
\declareconstitem{PMIX_JOB_STATE_TERMINATED}
...
%
\declareconstitem{PMIX_JOB_STATE_TERMINATED_WITH_ERROR}
The job has terminated and is no longer running - typically will
...
All processes in the job have connected to their \ac{PMIx} server
%
\declareconstitem{PMIX_JOB_STATE_UNTERMINATED}
...
\subsubsection{Initialize the \refstruct{pmix_cpuset_t} structure}
...
%%%%%%%
\subsection{Relative locality of two processes}
...
Session identifier assigned by the scheduler - referenced using \refconst{PMIX_RANK_WILDCARD}.
}
...
Comma-delimited list or regular expression of all nodes in this allocation regardless of whether or not they currently host processes - referenced using \refconst{PMIX_RANK_WILDCARD}.
}
...
%
\declareNewAttribute{PMIX_PROC_INFO_ARRAY}{"pmix.pdata"}{pmix_data_array_t}{
...
Number of allocated slots in a session - each slot may or may not be occupied by an executing process. Note that this attribute is equivalent to providing the \refattr{PMIX_MAX_PROCS} entry in the \refattr{PMIX_SESSION_INFO_ARRAY} array - it is included in the Standard for historical reasons.
}
...
Deprecated in favor of the \refattr{PMIX_PROC_INFO_ARRAY} attribute
}
...
%
\declareNewAttribute{PMIX_EVENT_TIMESTAMP}{"pmix.evtstamp"}{time_t}{
...
%
\declareNewAttribute{PMIX_NOTIFY_JOB_EVENTS}{"pmix.note.jev"}{bool}{
Requests that the \ac{PMIx} server provide the requester with the \refconst{PMIX_EVENT_JOB_START}, \refconst{PMIX_LAUNCH_COMPLETE}, and \refconst{PMIX_EVENT_JOB_END} events. Each event is to include at least the namespace of the corresponding job and a \refattr{PMIX_EVENT_TIMESTAMP} indicating the time the event occurred. Note that these events can be captured and processed by registering a default event handler instead of individual handlers and then processing the events based on the returned status code. Another common method is to register one event handler for all job-related events, with a separate handler for non-job events - see \refapi{PMIx_Register_event_handler} for details.
}

%
\declareAttribute{PMIX_NOTIFY_COMPLETION}{"pmix.notecomp"}{bool}{
Requests that the \ac{PMIx} generate the \refconst{PMIX_EVENT_JOB_END} event for normal or abnormal termination of the spawned job. The event shall include the returned status code for the corresponding job and a \refattr{PMIX_EVENT_TIMESTAMP} indicating the time the termination occurred.
}

%
\declareNewAttribute{PMIX_LOG_JOB_EVENTS}{"pmix.log.jev"}{bool}{
Requests that the \ac{PMIx} server log the \refconst{PMIX_EVENT_JOB_START}, \refconst{PMIX_LAUNCH_COMPLETE}, and \refconst{PMIX_EVENT_JOB_END} events using \refapi{PMIx_Log} instead of generating \ac{PMIx} events, subject to the logging attributes of Section \ref{api:struct:attributes:log}
}
...
Returns a (\refstruct{pmix_data_array_t}) array of \refstruct{pmix_proc_info_t}, one entry for each process in the specified namespace executing on the same node as the requester, ordered by process job rank. REQUIRED QUALIFIER: \refattr{PMIX_NSPACE} indicating the namespace whose process table is being queried
}
...
%
\declareDepAttribute{PMIX_ALLOC_NETWORK}{"pmix.alloc.net"}{array}{
Changed to \refattr{PMIX_ALLOC_FABRIC}
}
%
...
%
\declareDepAttribute{PMIX_ALLOC_NETWORK_ID}{"pmix.alloc.netid"}{char*}{
Changed to \refattr{PMIX_ALLOC_FABRIC_ID}
}
%
...
%
\declareDepAttribute{PMIX_ALLOC_NETWORK_QOS}{"pmix.alloc.netqos"}{char*}{
Changed to \refattr{PMIX_ALLOC_FABRIC_QOS}
}
...
%
\declareDepAttribute{PMIX_ALLOC_NETWORK_TYPE}{"pmix.alloc.nettype"}{char*}{
Changed to \refattr{PMIX_ALLOC_FABRIC_TYPE}
}
%
...
%
\declareDepAttribute{PMIX_ALLOC_NETWORK_PLANE}{"pmix.alloc.netplane"}{char*}{
Changed to \refattr{PMIX_ALLOC_FABRIC_PLANE}
}
%
...
%
\declareDepAttribute{PMIX_ALLOC_NETWORK_ENDPTS}{"pmix.alloc.endpts"}{size_t}{
Changed to \refattr{PMIX_ALLOC_FABRIC_ENDPTS}
}
%
...
%
\declareDepAttribute{PMIX_ALLOC_NETWORK_ENDPTS_NODE}{"pmix.alloc.endpts.nd"}{size_t}{
Changed to \refattr{PMIX_ALLOC_FABRIC_ENDPTS_NODE}
}
%
...
%
\declareDepAttribute{PMIX_ALLOC_NETWORK_SEC_KEY}{"pmix.alloc.nsec"}{pmix_byte_object_t}{
Changed to \refattr{PMIX_ALLOC_FABRIC_SEC_KEY}
}
%
