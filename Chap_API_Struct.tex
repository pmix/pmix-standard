...
Array of \refstruct{pmix_proc_t} of processes on the specified node - referenced using \refconst{PMIX_RANK_WILDCARD}.
}
...
The following attributes are used to specify the level of information (e.g., \refterm{session}, \refterm{job}, or \refterm{application}) being requested where ambiguity may exist - see \ref{} and \ref{} for examples of their use.
...
The following attributes are used to assemble information by its level (e.g., \refterm{session}, \refterm{job}, or \refterm{application}) for storage where ambiguity may exist - see \ref{} for examples of their use.
...
Number of allocated slots in a session - each slot may or may not be occupied by an executing process.
}
...
%
\declareNewAttribute{PMIX_NUM_SLOTS}{"pmix.num.slots"}{uint32_t}{
