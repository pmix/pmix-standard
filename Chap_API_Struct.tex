...
The constants defined in this section may be used before calling any library initialization routine.
Additional constants associated with specific data structures are defined in the section describing that data structure.
...
The \refstruct{pmix_rank_t} structure is an unsigned integer type for rank values.
...
The \refstruct{pmix_proc_state_t} structure is an unsigned integer type for process state values.
...
Define a ``boundary'' so users can easily and quickly determine if a process is still running or not.
...
The \refstruct{pmix_scope_t} structure is an unsigned integer type that defines the scope for data passed to \refapi{PMIx_Put}.
...
Data marked in this way will not be included in data packages sent to remote requestors, only processes on the local node.
%
\declareconstitem{PMIX_REMOTE}
The data is intended solely for applications processes on remote nodes.
Data marked in this way will not be shared with other processes on the same node, only processes on remote nodes.
%
...
The data is intended solely for this process.
The data is not shared with other processes.
%
\end{constantdesc}
...
The \refstruct{pmix_data_range_t} structure is an unsigned integer type that defines a range for ``published'' data.
...
The \refstruct{pmix_persistence_t} structure is an unsigned integer type that defines the policy for data published by clients.
...
The \refstruct{pmix_info_directives_t} structure is an unsigned integer type that defines the behavior of command directives via \refstruct{pmix_info_t} arrays.
...
If the user relies on the behavior defined by the \refstruct{pmix_info_t} then they must pass \refconst{PMIX_INFO_REQD} and then the PMIx implementation or \ac{RM} must return \refconst{PMIX_ERR_NOT_SUPPORTED} as soon as possible to the caller.
\adviceuserend
...
The \refstruct{pmix_alloc_directive_t} structure is an unsigned integer type that defines the behavior of allocation requests.
...
A value boundary beyond which implementers are free to define their own directive values.
%
\end{constantdesc}
...
\notestart
\noteheader
This structure is not currently used in the PMIx API.
\noteend
...
The \refstruct{pmix_data_type_t} structure is an unsigned integer type for identifying the data type for packing/unpacking purposes.
...
%
\declareconstitemDEP{PMIX_INFO_ARRAY}{2}
Info array
%
\end{constantdesc}
...
The \refstruct{pmix_value_t} structure is used to represent the value passed to \refapi{PMIx_Put}.

A collection of values may be specified under a single key by passing a \refstruct{pmix_value_t} containing an array of type \refstruct{pmix_info_array_t}, with each array element containing its own \refstruct{pmix_info_t} object.
...
\argin{src}{valu etype \refstruct{pmix_value_t} source (handle)}
\end{arglist}
...
Due to this extensibility the \refapi{PMIx_Get} API may return \refconst{PMIX_ERR_NOT_FOUND} if the provided \var{key} cannot be found.
...
Temporary directory where PMIx server will place \emph{client} rendezvous points and contact information.
}

%
\declareAttribute{PMIX_SYSTEM_TMPDIR}{"pmix.sys.tmpdir"}{char*}{
Temporary directory for this system, where PMIx server will place \emph{tool} rendezvous points and contact information.
}
...
These attributes are defined to identify a process and it's associated PMIx library.
...
How the \ac{URI} should be reported. \code{'-'} for stdout, \code{'+'} for stderr, or filename.
}
...
String describing a process's location.
}
...
Attributes used by host PMIx server to pass data to the PMIx server PMIx library.
...
Process mapping in \ac{ANL} notation (used in PMI-1/PMI-2).
}
...
\begin{codepar}
typedef void
(*pmix_release_cbfunc_t)(void *cbdata)
\end{codepar}
\cspecificend
...
\begin{codepar}
typedef void
(*pmix_modex_cbfunc_t)(pmix_status_t status,
...
\begin{codepar}
typedef void
(*pmix_spawn_cbfunc_t)(pmix_status_t status,
...
\begin{codepar}
typedef void
(*pmix_op_cbfunc_t)(pmix_status_t status, void *cbdata);
\end{codepar}
\cspecificend
...
\begin{codepar}
typedef void
(*pmix_lookup_cbfunc_t)(pmix_status_t status,
...
%%%%%%%%%%%
\section{Additional Support Functions}

This section describes some additional support APIs.
...
\ldots Do we need this in the standard, or is it just a useful feature of the PRL?
\ldots My initial thought would be to only include the PMIX_ARGV_APPEND macro in the standard - the function name is only how it is implemented. However, that function does appear in pmix_common.h, and so someone technically could call it directly instead of via the macro.
...
\ldots Should this go somewhere else?
