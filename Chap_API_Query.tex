...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Query structure support functions}
The following functions are provided to support the \refstruct{pmix_query_t} structure.

\littleheader{Initialize the query structure}
\declareapiProvisional{PMIx_Query_construct}

Initialize the \refstruct{pmix_query_t} fields

\copySignature{PMIx_Query_construct}{6.0}{
void \\
PMIx_Query_construct(pmix_query_t *p);
}

\begin{arglist}
\argin{p}{Pointer to the structure to be initialized (pointer to \refstruct{pmix_query_t})}
\end{arglist}

\littleheader{Destruct the query structure}
\declareapiProvisional{PMIx_Query_destruct}

Destruct the \refstruct{pmix_query_t} fields

\copySignature{PMIx_Query_destruct}{6.0}{
void \\
PMIx_Query_destruct(pmix_query_t *p);
}
...
\declareapiProvisional{PMIx_Query_create}

Allocate and initialize an array of \refstruct{pmix_query_t} structures

\copySignature{PMIx_Query_create}{6.0}{
pmix_query_t* \\
PMIx_Query_create(size_t n);
}
...
Returns pointer to the array of \refstruct{pmix_query_t} structures (handle)

%%%%%%%%%%%
\littleheader{Free a query structure}
\declareapiProvisional{PMIx_Query_release}

Release a \refstruct{pmix_query_t} structure

\copySignature{PMIx_Query_release}{6.0}{
void \\
PMIx_Query_release(pmix_query_t *p);
}
...
\declareapiProvisional{PMIx_Query_free}

Release an array of \refstruct{pmix_query_t} structures

\copySignature{PMIx_Query_free}{6.0}{
void \\
PMIx_Query_free(pmix_query_t *p,
\hspace*{24\sigspace}size_t n);
}

\begin{arglist}
\argin{p}{Pointer to the array of \refstruct{pmix_query_t} structures (handle)}
\argin{n}{Number of structures in the array (\code{size_t})}
\end{arglist}

%%%%%%%%%%%
\littleheader{Create the info array of query qualifiers}
\declareapiProvisional{PMIx_Query_qualifiers_create}
...
\copySignature{PMIx_Query_qualifiers_create}{6.0}{
pmix_info_t* \\
PMIx_Query_qualifiers_create(size_t n);
}
...
Returns pointer to the \refstruct{pmix_info_t} structure (handle)
