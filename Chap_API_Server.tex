...
%
\declareAttribute{PMIX_SINGLETON}{"pmix.singleton"}{char*}{
...
This is intended to support persistent \ac{PMIx} servers by providing an opportunity for the host \ac{RM} to tell the \ac{PMIx} server library to release all memory for a completed job. Note that a \code{NULL} \refarg{cbfunc} reference indicates that the function is to be executed as a blocking operation.
...
\begin{constantdesc}
%
\declareconstitemvalue{PMIX_ERR_REPEAT_ATTR_REGISTRATION}{-171}
...
\subsubsection{Attribute registration structure support functions}

The following functions are provided to support the \refstruct{pmix_regattr_t} structure.

%%%%%%%%%%%
\littleheader{Initialize the regattr structure}
\declareapiProvisional{PMIx_Regattr_construct}

Initialize the \refstruct{pmix_regattr_t} fields

\copySignature{PMIx_Regattr_construct}{6.0}{
void \\
PMIx_Regattr_construct(pmix_regattr_t *p);
}

\begin{arglist}
\argin{p}{Pointer to the structure to be initialized (pointer to \refstruct{pmix_regattr_t})}
\end{arglist}

%%%%%%%%%%%
\littleheader{Destruct the regattr structure}
\declareapiProvisional{PMIx_Regattr_destruct}


\copySignature{PMIx_Regattr_destruct}{6.0}{
void \\
PMIx_Regattr_destruct(pmix_regattr_t *p);
}

Destruct the \refstruct{pmix_regattr_t} fields, releasing all strings.

\begin{arglist}
\argin{p}{Pointer to the structure to be destructed (pointer to \refstruct{pmix_regattr_t})}
\end{arglist}

%%%%%%%%%%%
\littleheader{Create a regattr array}
\declareapiProvisional{PMIx_Regattr_create}

Allocate and initialize an array of \refstruct{pmix_regattr_t} structures.

\copySignature{PMIx_Regattr_create}{6.0}{
pmix_regattr_t* \\
PMIx_Regattr_create(size_t n);
}
...
Returns pointer to the array of \refstruct{pmix_regattr_t} structures (handle)

%%%%%%%%%%%
\littleheader{Free a regattr array}
\declareapiProvisional{PMIx_Regattr_free}

Release an array of \refstruct{pmix_regattr_t} structures.

\copySignature{PMIx_Regattr_free}{6.0}{
void \\
PMIx_Regattr_free(pmix_regattr_t *p, \\
\hspace*{24\sigspace}size_t n);
}

\begin{arglist}
\arginout{p}{Pointer to the array of \refstruct{pmix_regattr_t} structures (handle)}
\argin{n}{Number of structures in the array (\code{size_t})}
\end{arglist}

%%%%%%%%%%%
\littleheader{Load a regattr structure}
\declareapiProvisional{PMIx_Regattr_load}

Load values into a \refstruct{pmix_regattr_t} structure. The function can be called multiple times to add as many strings as desired to the same structure by passing the same address and a \code{NULL} key to the function. Note that the \refarg{t} type value must be given each time.

\copySignature{PMIx_Regattr_load}{6.0}{
void \\
PMIx_Regattr_load(pmix_regattr_t *p, \\
\hspace*{24\sigspace}const char *n, \\
\hspace*{24\sigspace}const char *k, \\
\hspace*{24\sigspace}pmix_data_type_t t, \\
\hspace*{24\sigspace}const char *v);
}
...
\declareapiProvisional{PMIx_Regattr_xfer}

Non-destructively transfer the contents of a \refstruct{pmix_regattr_t} structure to another one.

\copySignature{PMIx_Regattr_xfer}{6.0}{
void \\
PMIx_Regattr_xfer(pmix_regattr_t *p, \\
\hspace*{24\sigspace}const pmix_regattr_t *s);
}

\begin{arglist}
\arginout{p}{Pointer to the destination \refstruct{pmix_regattr_t} structure (handle)}
\argin{s}{Pointer to the source \refstruct{pmix_regattr_t} structure (handle)}
\end{arglist}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Cpuset support functions}
The following functions support the \refstruct{pmix_cpuset_t} structure.


\littleheader{Initialize the cpuset structure}
\declareapiProvisional{PMIx_Cpuset_construct}

Initialize the \refstruct{pmix_cpuset_t} fields.

\copySignature{PMIx_Cpuset_construct}{6.0}{
void \\
PMIx_Cpuset_construct(pmix_cpuset_t *m);
}
...
\declareapiProvisional{PMIx_Cpuset_destruct}

Destruct the \refstruct{pmix_cpuset_t} fields.

\copySignature{PMIx_Cpuset_destruct}{6.0}{
void \\
PMIx_Cpuset_destruct(pmix_cpuset_t *m);
}
...
\declareapiProvisional{PMIx_Cpuset_create}

Allocate and initialize a \refstruct{pmix_cpuset_t} array.

\copySignature{PMIx_Cpuset_create}{6.0}{
pmix_cpuset_t* \\
PMIx_Cpuset_create(size_t n);
}
...
Returns pointer to the array of \refstruct{pmix_cpuset_t} structures (handle)

\littleheader{Release a cpuset array}
\declareapiProvisional{PMIx_Cpuset_free}

Deconstruct and free a \refstruct{pmix_cpuset_t} array.

\copySignature{PMIx_Cpuset_free}{6.0}{
void \\
PMIx_Cpuset_free(pmix_cpuset_t *m, \\
\hspace*{24\sigspace}size_t n);
}

\begin{arglist}
\arginout{m}{Address of the array of \refstruct{pmix_cpuset_t} structures to be released (handle)}
