...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Lookup data structure support functions}

The following functions are provided to support the \refstruct{pmix_pdata_t} structure.

%%%%
\littleheader{Initialize the pdata structure}
\declareapiProvisional{PMIx_Pdata_construct}

Initialize the \refstruct{pmix_pdata_t} fields

\copySignature{PMIx_Pdata_construct}{6.0}{
void \\
PMIx_Pdata_construct(pmix_pdata_t *p);
}

\begin{arglist}
\argin{p}{Pointer to the structure to be initialized (pointer to \refstruct{pmix_pdata_t})}
\end{arglist}

\littleheader{Destruct the pdata structure}
\declareapiProvisional{PMIx_Pdata_destruct}

Destruct the \refstruct{pmix_pdata_t} fields

\copySignature{PMIx_Pdata_destruct}{6.0}{
void \\
PMIx_Pdata_destruct(pmix_pdata_t *p);
}

\begin{arglist}
\argin{p}{Pointer to the structure to be destructed (pointer to \refstruct{pmix_pdata_t})}
\end{arglist}

%%%%%%%%%%%
\littleheader{Create a pdata array}
\declareapiProvisional{PMIx_Pdata_create}

Allocate and initialize an array of \refstruct{pmix_pdata_t} structures

\copySignature{PMIx_Pdata_create}{6.0}{
pmix_pdata_t* \\
PMIx_Pdata_create(size_t n);
}
...
Returns pointer to the array of \refstruct{pmix_pdata_t} structures (handle)


%%%%%%%%%%%
\littleheader{Free a pdata structure}
\declareapiProvisional{PMIx_Pdata_release}

Release a \refstruct{pmix_pdata_t} structure

\copySignature{PMIx_Pdata_release}{6.0}{
void \\
PMIx_Pdata_release(pmix_pdata_t *p);
}

\begin{arglist}
\argin{p}{Pointer to a \refstruct{pmix_pdata_t} structure (handle)}
\end{arglist}


%%%%%%%%%%%
\littleheader{Free a pdata array}
\declareapiProvisional{PMIx_Pdata_free}

Release an array of \refstruct{pmix_pdata_t} structures

\copySignature{PMIx_Pdata_free}{6.0}{
void \\
PMIx_Pdata_free(pmix_pdata_t *p, \\
...
\declareapiProvisional{PMIx_Pdata_load}

This function simplifies the loading of key, process identifier, and data into a \refstruct{pmix_pdata_t} by correctly assigning values to the structure's fields.

\copySignature{PMIx_Data_load}{6.0}{
void \\
PMIx_Pdata_load(pmix_pdata_t *dest, \\
\hspace*{24\sigspace}const pmix_proc_t *p, \\
\hspace*{24\sigspace}const char *key, \\
\hspace*{24\sigspace}const void *data, \\
\hspace*{24\sigspace}pmix_data_type_t type);
}

\begin{arglist}
\argin{dest}{Pointer to the \refstruct{pmix_pdata_t} structure into which the key and data are to be loaded (pointer to \refstruct{pmix_pdata_t})}
...
\adviceuserstart
Key, process identifier, and data will all be copied into the \refstruct{pmix_pdata_t} - thus, the source information can be modified or free'd without affecting the copied data once the function has completed.
\adviceuserend

%%%%%%%%%%%
\littleheader{Transfer a lookup data structure}
\declareapiProvisional{PMIx_Pdata_xfer}
...
\copySignature{PMIx_Data_xfer}{6.0}{
void \\
PMIx_Pdata_xfer(pmix_pdata_t *d, \\
\hspace*{24\sigspace}const pmix_pdata_t *s);
}
...
\adviceuserstart
Key, process identifier, and data will all be copied into the destination \refstruct{pmix_pdata_t} - thus, the source \refstruct{pmix_pdata_t} may be modified or free'd without affecting the copied data once the function has completed.
\adviceuserend
