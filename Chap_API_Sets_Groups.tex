...
\emph{groups} are included in the process \emph{set} information returned by
calls to \refapi{PMIx_Query_info_nb} with the \refattr{PMIX_PSET_NAMES}.
Thus, a \emph{process group} can effectively be considered an extended version
of a \emph{process set} that adds dynamic definition and operational context
to the \emph{process set} concept.
